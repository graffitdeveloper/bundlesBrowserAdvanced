...
namespace AssetBundleBrowser {
	public class AssetBundleBrowserMain : EditorWindow, IHasCustomMenu, ISerializationCallbackReceiver {
		...
		[SerializeField] internal AssetBundleAdvancedBuildTab _mAdvancedBuildTab;

		[SerializeField] internal AssetBundleBuildTab _mBuildTab;
		...
		[MenuItem("Window/Asset Management/AssetBundle Browser", priority = 2050)]
		...
		private void OnDisable() {
			if (_mAdvancedBuildTab != null)
				_mAdvancedBuildTab.OnDisable();
			if (_mBuildTab != null)
				_mBuildTab.OnDisable();
			if (m_InspectTab != null)
		...
		void ModeToggle() {
			...
			string[] labels = new string[4] {"Configure", "Advanced Build", "Classic Build", "Inspect"};
			...
			if (multiDataSource) {
				...
				using (new EditorGUILayout.HorizontalScope(EditorStyles.toolbar)) {
					...
					var c = new GUIContent(
						string.Format("{0} ({1})", AssetBundleModel.Model.DataSource.Name, AssetBundleModel.Model.DataSource.ProviderName),
						"Select Asset Bundle Set");
